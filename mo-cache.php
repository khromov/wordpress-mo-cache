...
class MOCache
{
  ...
  public function load($override, $domain, $mofile)
  {
    ...
    //Add a cache global group, so we don't cache multiple times on multisite. This relies on object-cache.php, whichever you have.
    if(function_exists('wp_cache_add_global_groups'))
      wp_cache_add_global_groups(self::GROUP);

    //Generate object cache key name
    $key = md5('moc-'. self::VERSION . "-${GLOBALS['wp_version']}-$mofile");
